...
class NPMyPlanViewModelTest {
    ...
    fun setup() {
        ...
        viewModel = NPMyPlanViewModel(repository)
    }

    @Test
    fun `fetchMyPlanNP should return result successfully`() = runTest {
        repository.shouldReturnError = false
        val viewModel = NPMyPlanViewModel(repository)
        ...
        viewModel.state.test {
            ...
            Assert.assertEquals("R$ 99,90", success.result?.screen?.planValue)
    ...
    @Test
    fun `fetchMyPlanNP should handle error properly`() = runTest {
    ...
    fun `state should transition from loading to success`() = runTest {
        ...
        viewModel.state.test {
            ...
            Assert.assertEquals("R$ 99,90", result.screen?.planValue)
            Assert.assertEquals("Meu Plano", result.screen?.name)
